...
2026-08-27 사용자 보고로 K1 assembly, S0, VO617A-3, F2 holder/fuse와 6P/18 AWG harness도
도착했다. 이들은 `USER-REPORTED RECEIVED / INCOMING AND FIT OPEN`이며 지금 H-01에서 실측·
dry placement할 수 있다. S2와 P6KE16CA x3는 미도착이므로 해당 mounting freeze는 계속 HOLD다.
...
| --- | --- | --- | --- |
| K1 `V23134J1052D642` + `VCF7-1000` | adapter plate/chassis 쪽 별도 강성 bracket 우선 | Motor current와 AWG 12 main path는 perfboard를 통과하지 않는다. 기존 RevB에 K1 전용 장착 형상은 없다. | 입고한 relay+socket+terminal 전체 dry fit, bracket/retention, terminal 출구, AWG 12 굽힘·공구 접근 공간 |
...
| `VO617A-3`, `680 ohm`, `10 kohm` | permanent perfboard THT | PC7 E-stop sense conditioner로 K2 인접 저전류 영역에 둔다. | actual suffix/pin-1 방향, current perfboard dry fit와 이후 무전원 continuity |
...
| S0 `SF2ER-E2R2B-A` | 별도 operator panel | 사용자가 track을 건드리지 않고 즉시 누르고 의도적으로 해제할 수 있어야 한다. Main horizontal plate 내부 장착 대상으로 보지 않는다. | 입고품 marking, 2NC block, panel cutout/두께, rear depth와 배선 접근 |
...
| 6P E-stop harness connector | signal/control inline disconnect | S0-A/S0-B/S2 회로를 분리·점검하기 위한 connector다. Motor current를 운반하지 않는다. | mating-face cavity map, keying, 18 AWG terminal retention, bend/strain relief와 label |
...
사용자 도착 보고된 K1/VCF7 relay, socket, 280756-4/42281-1 terminal과 F1/F2 holder를
incoming 확인 뒤 ruler와 함께 촬영하고 실제 terminal 방향을 정한다. 그 다음에만 on-plate
bracket인지 별도 chassis bracket인지 결정한다.
...
Plate 수령으로 주문 critical path가 사라졌으므로 카페에서는 physical fit을 추정하지 않는다.
상태 문서와 fit-check 양식을 정정한 뒤 `P-02A` production differential-drive command mapper의
수식, 단위, saturation과 pure-function interface 설계로 이동한다.
