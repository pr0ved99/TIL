...
**문서 정리 기준: 2026-09-19.** 아래 상태는 기록된 검증 범위이며 현재 보드의 전원·배선 상태를 대신하지 않는다.

| 분야 | 확인된 범위 | 남은 핵심 검증 |
| --- | --- | --- |
| 통신·펌웨어 | UART 명령 처리, 오류·timeout 복구, 엔코더 및 출력 상태 데이터의 일부 보드 검증 | 비상정지 통합 시험과 시험 후 안전 이미지 복구 |
| 구동·피드백 | MCU/드라이버 입력의 PWM·DIR, 수동 엔코더 계측 | 실모터 무부하 구동, 좌우 대응과 전동 구동 중 노이즈 |
| 물리 비상정지 | 제어 회로의 유지·해제·자동 재시작 방지, 전원·감지 신호의 일부 기능 | 감지–펌웨어–PWM 결합, 실제 모터 전원 차단 경로와 정지 |
| 측정 배선 | UART·CTRL·ENC·IMU 배선, 무전원 도통/비도통 및 마감 검사 사용자 보고 PASS | 최신 양면 PDF, 새 배선을 통한 통신·센서 동작 검증 |
| 기구·주행 | 어댑터 플레이트 설계와 제작품 수령 기록 | 실물 장착, 배터리 ADC·저전압 동작, 저속 주행과 1 m 거리 비교 |

다음 순서는 **사용자 펌웨어 빌드·플래시 → 모터 분리 비상정지 통합 →
전원 경로 검증 → 기구 장착과 실모터·주행 검증**이다.
