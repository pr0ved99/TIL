...
Last updated: 2026-09-07 (session/documentation recovery; latest hardware evidence: 2026-09-05)
...
- The historical Physical E-stop perfboard RevC digital checkpoint is `09_Electrical_Design/VeroRoute/Tracked_Mobile_Robot_Perfboard_RevC_Estop_FINAL.vrt`, 122,915 bytes, SHA-256 `680999D4AF62FC953635B7AA56EE6DE31CD4CCCF2A39F3C9F8E2B7170A70DFF2`. The 2026-09-07 file audit found WIP at 122,116 bytes, SHA-256 `43FD79BACF3A0505C3FED0F5838F4569657CF7F893B19E956395CACB6A1D8848`; the previously documented 122,954-byte WIP is historical. The dated PDFs also differ from earlier hashes; current sizes/hashes are recorded in `09_Electrical_Design/VeroRoute/README.md`. Their exact source-VRT linkage, current mirror/scale and K2 coil-polarity rework have not been verified. Preserve the files as checkpoints; a verified corrected as-built revision/export/hash is required before fabrication reuse.
...
- The first powered K2 attempt measured about `12.22 V` at the assumed coil points but produced no pickup. Root cause was applying the Panasonic TX2 datasheet's **BOTTOM VIEW** numbering directly to the component side: physical pin 1(+) and pin 12(-) were reversed. The corrected component-side left end is upper pin 12/GND and lower pin 1/`K2_COIL_P`; the two contact poles remain functional as `9(COM)->8(NO)` for seal-in and `4(COM)->5(NO)` for K1 permission. After rewiring, a 12.24 V, MDD10A-B+-disconnected run held `JK1COIL.1` at `12.19 V` after S2 release, opened K2/K1 on S0 press, did not auto-restart on S0 release or S1 OFF->ON, and required a new S2 action. The integrated `JESTOP.3<->.4` S0-B NC truth table also passed unpowered. These are operator-reported control-only results; powered VO617A/PC7 LOW-HIGH/wire-open, firmware integration, direct downstream motor rail, load/thermal/transient and actual motor-stop tests remain open.
...
- As of 2026-09-05, S2/P6KE incoming screens, the 18 AWG 6P first article/full harness, cavity/pair mapping, continuity/isolation and retention are operator-reported PASS. K1/K2 nominal control-only pickup/dropout and no-auto-restart also passed after correcting K2 polarity. Remaining blockers are conditioned S0-B/VO617A/PC7 power testing, explicit integrated wire-open evidence, firmware/PWM integration, direct MDD10A downstream-rail evidence, F1/F2/14-AWG termination coordination and load/thermal/transient tests—not tooling delivery or loose-harness assembly.
...
- Physical connector, footprint, perfboard and harness release; 6P cavity mapping, 18 AWG first article and terminal retention passed in the operator-reported scope, while permanent labels, integrated wire-break/independence, seal/IP qualification and corrected digital as-built verification remain open
...
2. Read `docs/handoff/2026-09-07_session_recovery_handoff.md` and the latest progress for the recovered state. For bench work, use `docs/plans/2026-09-05_Physical_EStop_Remaining_Bench_Gates_ko.md`, `docs/progress/2026-09-05_progress.md` and report 24. Use the 2026-09-03 plan/progress and 2026-09-01 progress as historical RevC assembly records, then read the 2026-08-30/29 firmware checkpoints, reports 23/22/19~21 and the authoritative scope plan `docs/plans/2026-08-25_Final_MVP_Remaining_Work_and_Pre_Arrival_Plan_ko.md` as needed.
...
13. Follow the 2026-09-05 runbook: first close any still-unrecorded integrated sense-wire-open/control-sense independence case, then execute `T-ESTOP-003` powered VO617A/PC7 LOW-HIGH/open, `T-ESTOP-004` conditioned firmware/PWM latch and the remaining motor-disconnected direct-rail cases of `T-ESTOP-005A`. Keep direct-PC7 and K2/K1 control-only results as bounded subsets. Do not apply actual motor energy before all five MVP gates pass.
