...
- The 2026-09-08 XL4015 #1 distribution keeps `5V_NUC` and `5V_ESP` separate on the perfboard.
  NUCLEO endpoints are `C8,R28/E5V`, `C9,R28/GND`; ESP32 endpoints are `C32,R26/5V`,
  `C31,R26/GND`. Board landings are separate lower-left 2P connectors: NUC `C1,R31/R32` and ESP
  `C1,R34/R35`, each pin 1=GND and pin 2=+5 V. This deliberate separation allows individual board tests.
  XL4015 #1 OUT+/OUT- directly split into two 26 AWG cable pairs ending in the NUC and ESP 2P plugs;
  there is no inline 4P connector. Removing both 2P plugs from the board opens both positive branches for
  dual-USB mode; NUCLEO then returns to `JP5=PWR-U5V`,
  `JP1=open` while powered off. Connector identity/rating, wire construction/length/termination,
  startup/steady current, voltage drop and temperature remain release gates.
- The first `...XL4015_1_Dual_Board_Power_WIP.vrt` review is FAIL. NUCLEO reaches the correct
  `C8/R28` and `C9/R28` endpoints, but ESP `5V_ESP` was routed to `C32/R35` and return to `C31/R35`.
  Those are the ESP32 J3 GND/GND end holes; the intended J1 pair is `C32/R26=5V`, `C31/R26=GND`.
  At that time the two 2P board landings were mistakenly treated as a footprint mismatch. The later user
  clarification established that they are the independently removable NUC and ESP supply inputs fed directly
  by two branches from XL4015 #1.
- The user then overwrote the same WIP at 04:14 and corrected the ESP branch: current endpoints are
  `C32/R26=5V_ESP`, `C31/R26=GND`, while the lower R35 pair is disconnected from new power. NUCLEO
  remains correct and the two 5 V nodes remain separate from each other and `AUX_5V`; this digital-routing
  subset passes. A full VRT grid/wire graph check found zero dangling endpoints, asymmetric track bits,
  cross-Net edges or Wire endpoint count errors in the new power paths. The intended dual-2P board topology
  also passes digitally. The user subsequently reported completing the new board-path soldering,
  all three instructed residual-voltage checks at 0 V and continuity-mode isolation verification. Record the
  zero-volt/isolation gross-short subset as operator-reported PASS without inventing individual Ω/OL values. The
  user then reported all four branch and two common-GND continuity paths passing, which closes the new board-path
  post-solder unpowered Gate as operator-reported PASS in the passive-board/two-2P-removed scope. Dual-2P
  source polarity/continuity and USB-free NUCLEO/ESP32 individual/combined powered tests also passed. Pre-connect
  was `5.02 V`; individual OUT was `4.97 V`; combined OUT/NUC E5V/ESP 5V was `4.95/4.94/4.95 V`,
  and power-off rails were all `0 V`. Current/drop/temperature and solder-side photo evidence remain open.
  When both 2P plugs are attached, `5V_NUC` and `5V_ESP` continuity through XL4015 #1 OUT+ is expected;
  dual-USB mode requires removing both plugs, not merely switching #1 OFF.
...
- The current input distribution is `LiPo+ -> F1 -> S1 -> S1 OUT/VBAT_PROTECTED`, then branches to
  K1 pin 30, XL4015 #1 IN+, XL4015 #2 IN+ and `F2 -> 6P.1 -> S0-A -> 6P.2 -> JESTOP.2`.
  `JESTOP.1` is unused with no 1-2 jumper. LiPo- goes to `PWR_GND`, both buck IN- and the MDD return/common ground.
...
- Two-motor coordination now uses `2.88 A` rated-total, `18 A` simultaneous stall at 12 V and a conservative `18.9 A` full-charge estimate at 12.6 V. TE Connectivity `V23134J1052D642` / `1393304-9` (12 V, 1 Form A NO) with `VCF7-1000`/`1393310-4` socket, `280756-4` main terminals x2 and `42281-1` coil terminals x2 was ordered on 2026-08-18 and received. On 2026-08-28 the exact parts were matched, coil resistance measured `89.5 ohm` against the official `81~99 ohm` band, the de-energized NO contact was open and the coil-contact low-voltage gross-short screen was open. On 2026-09-05 the user assembled the K1 socket with 18 AWG coil leads, 14 AWG `30/87` leads and a bidirectional `P6KE16CA` across `85/86`; terminal retention/end-to-end continuity passed and the assembled coil measured `91~92.4 ohm`. A 12.24 V, motor-disconnected control-only run then exercised K1 pickup/dropout through K2. Catalog ratings still numerically cover the 18.9 A envelope, but the 14 AWG-to-`280756-4` termination basis, actual MDD10A downstream rail, motor-load waveform, voltage-drop/thermal and release timing remain open. Panasonic `ACA14535` remains a comparison benchmark, not the procured K1.
...
- F2 is the ordered Littelfuse `0287001.PXCN` 1 A ATOF with `FHAC0001ZXJA` holder. The received set passed the operator-reported unpowered continuity/movement screen on 2026-08-28, but the exact physical marking was not captured in immutable evidence and its powered time-current, voltage-drop and thermal behavior remain open. The 6P item began as a loose male/female waterproof connector kit; on 2026-09-05 the user completed the 18 AWG first article and full assembly, terminal retention, straight-through continuity/isolation and switch truth tables. The fixed cavity assignment is `1-2=S0-A NC`, `3-4=S0-B NC`, `5-6=S2 momentary NO`. Three `P6KE16CA-E3/54` coil-clamp candidates arrived; exact `CA` marking, no stripe and both-direction continuity/diode-mode open passed the identity/gross-short screen, and one was installed across K1 `85/86`. Clamp waveform/energy and loaded K1 release timing remain open.
- The historical Physical E-stop perfboard RevC digital checkpoint is `09_Electrical_Design/VeroRoute/Tracked_Mobile_Robot_Perfboard_RevC_Estop_FINAL.vrt`, 122,915 bytes, SHA-256 `680999D4AF62FC953635B7AA56EE6DE31CD4CCCF2A39F3C9F8E2B7170A70DFF2`. The initial 2026-09-07 WIP audit (`122,116 bytes / 43FD79BA...`) is historical after the user's later saves. Current WIP is 122,124 bytes with the old K2 label order; `Tracked_Mobile_Robot_Perfboard_RevC_Estop_K2_Label_Corrected_WIP.vrt` is the current label-corrected working file, 122,124 bytes, SHA-256 `13BD2C696521758112B567EC36912EF341935BB69EA2C3DD0B567B72EBF92FC1`. Its stored K2 labels are `12,10,9,8,1,3,4,5`; the user's component-side screenshot shows R19=`12/10/9/8`, R21=`1/3/4/5`, matching the existing runbook table. The user attributed the drawing error to applying datasheet bottom-view numbering directly to the component side. Label correction is verified; physical pad identity/continuity and full as-built/source-PDF linkage are not. Earlier bench values remain raw because actual probe contacts were not verified. Use the latest dated progress/runbook status for the next bench step. On 2026-09-08 the user ended additional NC/NO relay retests; retain existing nominal relay PASS and R02-R05 continuity results and mark R06-R09 skipped rather than passed. The subsequent Gate 2 wire-removal/independence checks are now an operator-reported functional PASS. Current artifact sizes/hashes and older checkpoints are recorded in `09_Electrical_Design/VeroRoute/README.md`.
- On 2026-09-05 the user reported completing the RevC perfboard soldering and the plan's local unpowered continuity/isolation checks. Final in-circuit values included R14 `9.94 kOhm`, U1 LED approximately `0.872 V` forward/open reverse, K2 coil `1.025 kOhm` and R9~R12 `10.08/10.35/9.95/9.77 kOhm`; session photos were reviewed but not copied into the repository. The 6P and external control path were then integrated as `S1 OUT -> F2 -> 6P.1 -> S0-A NC -> 6P.2 -> JESTOP.2`, leaving board `JESTOP.1` unused; `JESTOP.1` must never be jumpered to `.2`. `6P.3/.4` carry S0-B and `6P.5/.6` carry S2.
- On 2026-09-08 the user completed the remaining unpowered S0-A/S0-B wire-open independence sequence: both restored baselines had continuity; opening S0-B opened only `JESTOP.3<->.4`; restoring it recovered continuity; opening S0-A opened only `S1 OUT<->JESTOP.2`; final restoration recovered both paths. This is an operator-reported functional Gate 2 PASS. Exact removed cavities/terminations, numeric Ω/OL and photos were not provided, so evidence metadata remains incomplete. Do not repeat the completed nominal switch/relay tests. The user also confirmed the old direct D9/PC7-to-GND stimulus jumper is absent; that preflight item is closed.
- On 2026-09-08 the user confirmed there is no separate direct D9/PC7-to-GND jumper. XL4015 #1 now supplies the USB-free NUCLEO/ESP32 branches and XL4015 #2 supplies `R13.1` through `J3.1`; their OUT+ rails remain separate. #2 measured `5.03 V` no-load and `5.08 V` at J3. With STM32 3V3 at `3.30 V`, conditioned `ESTOP_SENSE` measured released `0.06 V`, pressed/latched `3.27 V`, released again `0.06 V`, and S0-B conductor-open `3.27 V steady`. These pass the `0.99/2.31 V` LOW/HIGH thresholds in the operator-reported voltage-function scope. LED-loop current, instrument/photo/raw metadata and exact restored voltage remain open, so formal `T-ESTOP-003` evidence is PARTIAL. `T-ESTOP-004` firmware/PWM integration was not run.
- The first powered K2 attempt measured about `12.22 V` at the assumed coil points but produced no pickup. Root cause was applying the Panasonic TX2 datasheet's **BOTTOM VIEW** numbering directly to the component side: physical pin 1(+) and pin 12(-) were reversed. The corrected component-side left end is upper pin 12/GND and lower pin 1/`K2_COIL_P`; the two contact poles remain functional as `9(COM)->8(NO)` for seal-in and `4(COM)->5(NO)` for K1 permission. After rewiring, a 12.24 V, MDD10A-B+-disconnected run held `JK1COIL.1` at `12.19 V` after S2 release, opened K2/K1 on S0 press, did not auto-restart on S0 release or S1 OFF->ON, and required a new S2 action. The integrated `JESTOP.3<->.4` S0-B NC truth table also passed unpowered. These are operator-reported control-only results; conditioned voltage-function passed later, while firmware/PWM integration, direct downstream motor rail, load/thermal/transient and actual motor-stop tests remain open.
...
- As of 2026-09-08, S2/P6KE incoming screens, the 18 AWG 6P harness, cavity/pair mapping, continuity/isolation and retention are operator-reported PASS. K1/K2 nominal control-only pickup/dropout and no-auto-restart passed after correcting K2 polarity; S0-A/S0-B wire-open independence is a functional PASS. The new XL4015 #1 dual-2P board/source paths and individual/combined board power passed, and XL4015 #2 powered the conditioned S0-B/VO617A/PC7 voltage-function PASS. Remaining blockers include firmware/PWM integration, direct MDD10A downstream-rail evidence, F1/F2/14-AWG termination coordination, load/thermal/transient tests and missing raw evidence—not tooling delivery or loose-harness assembly.
...
- Firmware learning workflow is user-authored: Codex inspects the current source and history, explains one small
  code change with its location and reason, the user types and saves it, and Codex reviews the saved result.
  Build, flash and bench proceed when the user directs. Codex does not write a complete firmware harness unless
...
- `09_Electrical_Design/01_Perfboard_Low_Current_Allocation_Plan_ko.md`: historical 150 x 100 mm/55 x 37-hole low-current zone planning baseline; current physical status and corrected K2 mapping are recorded in report 24, the 2026-09-05 runbook/progress and the VeroRoute README, with digital as-built verification still open
...
- `09_Electrical_Design/07_Perfboard_Digital_Layout_Workflow_Decision_ko.md`: adopted pre-solder 1:1 component/solder-side and KiCad-net-to-hole review Gate; workflow progressed through the RevC VeroRoute FINAL/PDF checkpoint
...
- `docs/progress/2026-09-01_progress.md`: historical RevC FINAL/PDF and partial-solder checkpoint; its open assembly/continuity status and artifact hashes were superseded by later work
- `docs/progress/2026-08-30_progress.md`: previous firmware/tooling checkpoint; canonical `29/29`, default-off reset-harness source/static + ESP isolated build PASS, reset-harness board runtime/target flash OPEN and VH-30J/WX-03B arrived-but-uninspected boundary
- `docs/progress/2026-08-29_progress.md`: historical P-04A completion and P-04B reason/command-age/direct-PC7 active-latch checkpoint, including the pre-reset-harness `28/28` and paired hook-0 isolated-build boundary
...
- `assets/logs/firmware_build/2026-08-29_p04b_hook0_isolated_build_pass.md`: pre-reset-harness all-hooks-`0U` isolated STM32/ESP32 build run, retained artifact hashes and explicit no-flash/no-runtime boundary
...
- On 2026-08-29, P-04B added STM32 TEL `reason/command_age_ms` and the matching ESP32 required strict parser/log path; canonical host/static discovery reached the now-historical `28/28 PASS` checkpoint. Controlled run02 verified the no-CMD sentinel, successful-CMD-only age reset, 500 ms `CMD_TIMEOUT`, ARM-only timeout and fresh-CMD recovery. Independent direct-PC7 run03 verified `ESTOP_ACTIVE`; run04 verified `ESTOP_ACTIVE -> ESTOP_LATCHED`, with software-cached PWM `0/0` throughout FAULT. The authoritative report is `docs/verification/23_P04B_Stop_Reason_and_Command_Age_Telemetry_Runtime_Test_Report_2026-08-29_ko.md`, with raw logs under `assets/logs/esp32_uart_bridge/2026-08-29_p04b_*.txt`. The pre-reset-harness all-hooks-`0U` isolated build run `20260829043337-25400-bc21` passed STM32 `0 errors / 0 warnings` and ESP32-S3 image generation, with hashes recorded in `assets/logs/firmware_build/2026-08-29_p04b_hook0_isolated_build_pass.md`; those images were not reflashed or checked in a no-command board runtime.
- Later on 2026-08-29, the default-off `BRIDGE_P04B_ESTOP_RESET_TEST_ENABLED=0U` closeout harness added one firmware contract, raising current canonical discovery to `25 + 2 + 2 = 29/29 PASS`; current ESP32 isolated build also passed. The harness has not been enabled/flashed/run on the boards, so active reset reject, released reset success and final all-hooks-`0U` target reflash/no-command safe runtime remain `OPEN`, and P-04B remains `PARTIAL`. Optional `PC -> ESP32` forwarding and battery remain pending.
...
- On 2026-08-28 K1 exact-part matching, `89.5 ohm` coil, de-energized-NO and coil-contact gross-short screens passed; S0 two-channel NC/latch states, S2 `3–4` momentary-NO action, VO617A-3 diode/input-output gross-short, P6KE16CA x3 identity/gross-short and F2 operator-reported continuity/movement passed. These are unpowered low-voltage screens, not insulation-withstand, powered integration or rail-off evidence. The 6P kit was unassembled at that checkpoint; the 18 AWG first article/full harness, pair mapping and retention subsequently passed as operator-reported evidence in report 24.
- A `VH-30J` interchangeable-die crimper set with seller-listed `WX-35WF (10~35 mm2)`, `WX-03B (0.5~6 mm2)`, `WS-25WF (2x0.5~2x6 mm2)` and `WS-692 (1.5~6 mm2)` dies arrived and was used for the operator-reported 18 AWG first article/6P assembly. Exact tool/die identity documentation and manufacturer-qualified crimp/termination approval remain open; printed wire-range coverage and hand-pull/continuity checks do not establish that approval. K1 main leads are as-built 14 AWG, with the terminal/wire-range deviation still open.
...
4. Preserve the all-hooks-`0U` current source, TIM4 period `4420`, current host/static `29/29`, the default-off P-04B reset-harness source/static and current ESP32 isolated build PASS, P-04A hook-0 50-TEL safe UART run, P-04B controlled reason/age and direct-PC7 active/latch subset, final perfboard 5 s all-LOW, P-03 historical safe-restore 10 s all-LOW, canonical 500 ms run03 and post-run03 safe run04 UART/D0~D3 evidence. The earlier pre-harness isolated STM32/ESP32 build and UART-safe artifact hashes plus `15/15`, `20/20`, `23/23`, `24/24`, `25/25`, `26/26`, `27/27`, `28/28` checkpoints remain historical. Harness-enabled board flash/runtime and the final all-hooks-`0U` target reflash/no-command safe runtime are still open; host/static/build PASS does not replace board or electrical evidence.
...
12. Preserve report 24's completed RevC local unpowered checks, 18 AWG 6P mapping/retention, K1 assembly and the 12.24 V motor-disconnected control-only latch/dropout/no-auto-restart subset. Do not reuse the frozen RevC FINAL/PDF as the K2 as-built pin source; create a new corrected revision that uses the TX2 bottom-view convention correctly and records board JESTOP.1 as unused.
13. Preserve the completed wire-open independence, XL4015 #1/#2 and conditioned `ESTOP_SENSE` voltage-function results in report 25. Next execute `T-ESTOP-004` with the user-authored learning workflow, then the remaining motor-disconnected direct-rail cases of `T-ESTOP-005A`. Keep direct-PC7 and K2/K1 control-only results as bounded subsets. Do not apply actual motor energy before all five MVP gates pass.
...
17. Preserve the KiCad `RevA DRAFT` history, `RevB-WIP` verified/TBD boundary and frozen RevC VeroRoute checkpoint as history. The physical RevC board and control-only subset advanced, but the K2 pin-view correction means a new verified as-built digital revision is required; do not call the board electrically released until the conditioned sense, firmware, direct-rail, load/thermal and motor gates pass.
