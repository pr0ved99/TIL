...
- A 2026-08-25 source audit found that normal production `CMD(vx,w)` was not mapped to left/right PWM/DIR. The existing raw motor-output call remains reachable only through a disabled controlled test hook, and current TEL motor/battery fields are placeholders. P-02A fixed a normalized, coupled-saturation open-loop mixer and pure-function/vector contract. On 2026-08-27 `drive_command_mapper.h/.c` was implemented, compiled by the CubeIDE ARM toolchain and included in a full STM32 Debug build with `0 errors, 0 warnings`; independent mapper vectors plus the C source contract then passed as part of the P-02B `23/23` checkpoint. P-02C-1 added `motor_output_set_signed()` with provisional forward/reverse polarity, fail-safe range/error handling and a static source contract. A user-run CubeIDE incremental build explicitly recompiled `motor_output.c` and linked with `0 errors, 0 warnings`; the unused `.text.motor_output_set_signed` section appeared at address `0` because `--gc-sections` discarded it while no caller exists. Current canonical host/static is `24/24`. P-02C-2 production protocol/state caller remains open. This is not calibrated mm/s or runtime motor evidence.
...
- `docs/progress/2026-08-27_progress.md`: current continuation state; partial parts arrival, P-02B completion, P-02C-1 signed-adapter/static/incremental-build evidence, canonical `24/24` and immediate incoming-inspection boundary
...
4. Preserve the all-hooks-`0U` current source, TIM4 period `4420`, current host/static `24/24` and final 5 s D0~D3 all-LOW capture. Earlier UART-safe artifact hashes and `15/15` checkpoints remain historical; host/static PASS does not replace board or electrical evidence.
...
9. `P-01` is complete through Accepted ADR-015. `P-02A/P-02B` design, HAL-independent source, independent vectors/static source contract and full STM32 build are complete. `P-02C-1` signed adapter source/static contract and incremental build are also complete; the immediate software task is `P-02C-2` production protocol/state caller integration. Plate identification/dry fit remains a separate home `H-01` checkpoint.
