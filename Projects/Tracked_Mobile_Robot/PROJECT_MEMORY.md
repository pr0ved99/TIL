...
- Current procured/selected Physical E-stop parts are Autonics `SF2ER-E2R2B-A` for S0, IDEC `ABW110G` for S2, Panasonic `TX2-12V` for K2 and Vishay `VO617A-3` for S0-B conditioning. K2 incoming unpowered screening passed. On 2026-08-28 S0 body/contact markings, two independent NC contact states and mechanical latch operation passed unpowered screening; VO617A-3 forward/reverse diode behavior and input-output isolation also passed. S2 remains not received, and neither result is a powered or integrated-circuit PASS.
...
- Two-motor coordination now uses `2.88 A` rated-total, `18 A` simultaneous stall at 12 V and a conservative `18.9 A` full-charge estimate at 12.6 V. TE Connectivity `V23134J1052D642` / `1393304-9` (12 V, 1 Form A NO) with `VCF7-1000`/`1393310-4` socket, `280756-4` main terminals x2 and `42281-1` coil terminals x2 was ordered on 2026-08-18 and received. On 2026-08-28 the exact parts were matched, coil resistance measured `89.5 ohm` against the official `81~99 ohm` band, the de-energized NO contact was open and coil/contact cross-isolation screened open. Catalog ratings still numerically cover the 18.9 A envelope. Socket retention with crimped terminals, suppression, powered pickup/dropout, actual motor-load waveform, voltage-drop/thermal and rail-off bench release remain open. Panasonic `ACA14535` remains a comparison benchmark, not the procured K1.
...
- P-03A/P-03B then moved timeout enforcement into a pre-RX helper that stops all output, zeros stored `vx/w` and enters `DISARMED`; accepted `ARM` resets the first-CMD window to the default 300 ms and current tick. Current canonical host/static is `22 + 2 + 2 = 26/26`; the forced 32-object ARM build exited 0 with no warning/error diagnostics and ELF `text=29268`, `data=172`, `bss=2832`. This closes source/static/full-build only. No new flash, board runtime or PWM waveform exists; channel/polarity remain provisional, TEL PWM fields remain zero placeholders, and P-03 target runtime remains pending.
- As of 2026-08-28, the remaining unreceived electrical components are S2 `ABW110G` and three `P6KE16CA-E3/54`; the ordered crimp tooling is also not received. The received K1/S0/VO617A-3/F2 subset has completed the explicitly recorded unpowered screens, and the loose 6P kit has completed inventory/visual screening only. S2 and clamp arrival still blocks complete nominal integration and powered coil tests.
...
- `docs/progress/2026-08-28_progress.md`: current continuation state; K1/S0/VO617A-3/F2 unpowered screening, loose 6P kit/tooling state, remaining S2/P6KE blockers and next home/cafe work split
- `docs/verification/19_Physical_EStop_Received_Component_Incoming_Precheck_2026-08-28_ko.md`: K1/S0/VO617A-3/F2 unpowered incoming evidence and 6P loose-kit/tooling boundary
...
- The pre-ADR-015 timeout candidate kept `ARMED` after output zero. ADR-015 supersedes it with output/stored command zero -> `DISARMED` -> accepted `ARM` + valid `CMD`. P-03A/P-03B source/static/full-build implements that transition and a reset default 300 ms first-CMD window after `ARM`; it does not implement sequence/session anti-replay, and flash/board/PWM runtime closure is still pending.
...
- On 2026-08-28 K1 exact-part matching, `89.5 ohm` coil, de-energized-NO and cross-isolation screens passed; S0 two-channel NC/latch states passed; VO617A-3 diode and input-output isolation screens passed; and F2 operator-reported continuity/movement passed. These are unpowered component screens, not powered integration or rail-off evidence. The 6P item is a loose connector kit and separate 18 AWG wire; only inventory/visual screening is complete. S2 `ABW110G` and `P6KE16CA-E3/54` x3 remain not received.
...
2. Read `docs/progress/2026-08-28_progress.md`, `docs/verification/19_Physical_EStop_Received_Component_Incoming_Precheck_2026-08-28_ko.md`, `docs/plans/2026-08-26_Pre_Arrival_Schedule_ko.md` and the authoritative scope plan `docs/plans/2026-08-25_Final_MVP_Remaining_Work_and_Pre_Arrival_Plan_ko.md` first. Treat report 18 as the earlier direct-PC7/K2/F1 hardware baseline and the 2026-08-18 handoff as history.
...
4. Preserve the all-hooks-`0U` current source, TIM4 period `4420`, current host/static `26/26` and final 5 s D0~D3 all-LOW capture. Earlier UART-safe artifact hashes and `15/15`, `20/20`, `23/23`, `24/24`, `25/25` checkpoints remain historical; host/static PASS does not replace board or electrical evidence.
...
9. `P-01` is complete through Accepted ADR-015. `P-02A~P-02C-2` and `P-03A/P-03B` source/static/full-build checkpoints are complete; P-03 motor/LiPo-disconnected target runtime remains open. Plate identification/dry fit remains a separate home `H-01` checkpoint.
10. Prepare P-03 target runtime, then continue real telemetry (`P-04`), battery sensing/low-voltage behavior (`P-05`) and wheel-distance/odometry (`P-06`) without claiming board-runtime evidence before it is collected.
...
12. Preserve report 19's completed K1/S0/VO617A-3/F2 unpowered screens. Confirm the 6P mating-face cavity numbers/orientation, then after the ordered crimper arrives qualify one spare 18 AWG terminal/seal first article by visual, pull and housing-retention checks. Inspect S2/P6KE16CA the same way when they arrive. Remove the temporary direct PC7-to-GND jumper before connecting the optocoupler path.
