...
- `docs/progress/2026-08-29_progress.md`: current continuation state; P-04A applied-output TEL/ESP parser implementation, canonical `27/27`, target UART runtime, hook-0 safe restore and remaining evidence boundary
...
2. Read `docs/progress/2026-08-29_progress.md`, report 22, then reports 19~21 and the authoritative scope plan `docs/plans/2026-08-25_Final_MVP_Remaining_Work_and_Pre_Arrival_Plan_ko.md` first. Treat the 2026-08-26 pre-arrival schedule, report 18 and the 2026-08-18 handoff as history.
...
4. Preserve the all-hooks-`0U` current source, TIM4 period `4420`, current host/static `27/27`, P-04A hook-0 50-TEL safe UART run, final perfboard 5 s all-LOW, P-03 historical safe-restore 10 s all-LOW, canonical 500 ms run03 and post-run03 safe run04 UART/D0~D3 evidence. Earlier UART-safe artifact hashes and `15/15`, `20/20`, `23/23`, `24/24`, `25/25`, `26/26` checkpoints remain historical; host/static PASS does not replace board or electrical evidence.
...
10. Preserve run04 as the P-03/REQ-SAFE-004 safe board baseline and report 22 as the P-04A applied-output/hook-0 UART baseline. Continue P-04B E-stop/timeout reason and command-age telemetry, then battery sensing/low-voltage behavior (`P-05`) and wheel-distance/odometry (`P-06`). Plate identification/dry fit remains a separate home `H-01` checkpoint.
