...
### 다음 LOW/HIGH 검사의 판정 기준 — 아직 미측정

아래는 다음 시험을 위한 기준 보완이며, 앞서 완료한 저항·도통·+5.05V 검사 결과를 변경하지 않는다.
검정 프로브는 각 JDBG_ENC Pin3(GND), 빨강은 Pin1(A) 또는 Pin2(B)에 둔다.
STM32를 분리하고 기존 1kΩ 직렬/15kΩ 풀다운을 유지한 MCU 측 노드에서 측정한다.

- 디지털 입력 기준: **LOW ≤ 0.3×VDD, HIGH ≥ 0.7×VDD**.
  VDD=3.30V 기준으로 LOW ≤ **0.99V**, HIGH ≥ **2.31V**다. 실제 보드 VDD를 기준값과 함께 기록한다.
  근거는 [STM32F446 데이터시트](../../assets/stm32f446mc.pdf) DS10693 Rev 10, pp.116–117, Table 56의 CMOS 입력 기준이다.
- 프로젝트의 입력 상한: 조정부 뒤 HIGH는 **3.3V 이하**여야 한다.
  [기존 엔코더 검사 중단 조건](../../02_Hardware_Validation/04_Encoder_Signal_Safety_Test.md#stop-conditions)을 유지한다.
  이는 핀의 절대최대 정격과 같은 뜻이 아니다.
- 네 채널 각각 축을 조금씩 움직여 멈추며 LOW와 HIGH를 모두 기록한다. 기준 사이에 머물거나
  상한을 넘으면 보드를 장착하지 않고 배선·부하 조건을 확인한다. 판정 경계에 걸린 값은 계측 오차를 확인한 뒤 판단한다.
- 과거 3.06~3.07V는 당시 배선의 관측값이며 이번 예상 합격값으로 복사하지 않는다.
  DMM 통과는 정적 전압 확인 범위다. 구동 중 잡음·과도전압·파형 검증까지 완료한 것으로 확대하지 않는다.

| 채널 | 측정 노드 | LOW / HIGH | 상태 |
| --- | --- | --- | --- |
| 왼쪽 A / PB4 | JDBG_ENC_1 Pin1 | 미측정 / 미측정 | 다음 작업 |
| 왼쪽 B / PB5 | JDBG_ENC_1 Pin2 | 미측정 / 미측정 | 다음 작업 |
| 오른쪽 A / PA0 | JDBG_ENC_2 Pin1 | 미측정 / 미측정 | 다음 작업 |
| 오른쪽 B / PA1 | JDBG_ENC_2 Pin2 | 미측정 / 미측정 | 다음 작업 |
