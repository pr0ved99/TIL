...
| `MVP-002` | ESP32 또는 PC가 동일한 protocol의 command source로 동작한다. | MUST | `PARTIAL` — exact startup, bounded loss, stale-seq/reset과 T-BRIDGE-007/008 required runtime PASS; all-hooks-`0U`, current host/static suites `23/23`, motor-output safety 시험 뒤 final exact startup과 post-READY TEL 155/155 over 15.4 s PASS; exact board-artifact linkage, external cold-start marker와 log-embedded physical setup provenance pending |
| `MVP-003` | 전원 경로와 MDD10A가 단계적으로 안전 검증된다. | MUST | `PARTIAL` |
| `MVP-004` | STM32가 좌우 MDD10A용 PWM/DIR 신호를 안전 규칙에 맞게 생성한다. | MUST | `PARTIAL` — raw controlled output의 핀/주파수/direction safety와 P-02B mapper source/full build/host-static vectors는 PASS했지만 normal production `CMD(vx,w)` caller와 signed request-to-PWM/DIR adapter의 P-02C 통합은 미구현 |
...
| ESP32 bridge | 동일 UART rule set을 ESP32 command source에서도 만족 | `PARTIAL` | Exact startup, bounded loss, stale response/reset recovery와 T-BRIDGE-007/008 required runtime PASS; all-hooks-`0U`, current host/static suites `23/23`와 final safe UART runtime PASS; exact artifact/setup provenance 대기 |
...
Software 쪽은 P-02B mapper source/full build, independent vectors와 C source 정적 계약까지
완료해 current host/static `23/23`을 통과했다. P-02C production caller/output integration과
...
| 2 | `T-COM-002` | ESP32-STM32 UART bridge | `T-COM-001` | `PARTIAL` — Gate A/B와 T-BRIDGE-007/008 required runtime PASS; all-hooks-`0U`, current host/static `23/23`, post-motor-safety final exact startup과 post-READY TEL 155/155 over 15.4 s safe runtime PASS; exact board-artifact linkage, external cold-start marker와 log-embedded physical setup provenance pending |
