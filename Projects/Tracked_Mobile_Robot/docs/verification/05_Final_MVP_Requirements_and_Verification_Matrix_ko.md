...
| `REQ-ESTOP-005~008` | 독립 auxiliary NC sense, software latch, boot-safe와 explicit-reset/no-auto-restart를 만족해야 한다. | MUST | `PARTIAL/BLOCKED` — direct-PC7 latch/reset과 healthy-path hardware no-auto-restart subsets PASS; S0-B 5 V conditioned path, active-output assertion and combined hardware/firmware acceptance pending |
...
| `REQ-ESTOP-017~020` | Back-power 방지, harness 식별, 안전한 시험환경과 완전한 evidence record를 만족해야 한다. | MUST | `PARTIAL/BLOCKED` — 6P 18 AWG assembly/cavity/truth-table/retention과 motor-disconnected control-relay subset은 operator-reported PASS. Tool qualification, direct MDD rail/back-power, immutable raw evidence and motor acceptance pending |
