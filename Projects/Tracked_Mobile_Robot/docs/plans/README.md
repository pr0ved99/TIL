...
Current firmware checkpoint is `P-04B PARTIAL`, with canonical host/static `28/28` and the
reason/command-age runtime subset complete. The closeout order is active reset rejection
...
The hook-0 isolated STM32/ESP32 build and artifact hashes are already PASS.

## Index

| Date range | File | Scope |
| --- | --- | --- |
| Project-wide / current | [`00_Project_Master_Plan_To_Final_MVP_ko.md`](00_Project_Master_Plan_To_Final_MVP_ko.md) | Four-chapter V-model roadmap; P-04B PARTIAL/current `28/28` closeout sequence, nominal E-stop MVP gate and post-MVP single-fault boundary |
